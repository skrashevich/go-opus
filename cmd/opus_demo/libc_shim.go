...
func xcalloc(tls *libc.TLS, n, size uint64) uintptr {
	return libc.Xcalloc(tls, libc.Tsize_t(n), libc.Tsize_t(size))
}
...
func xfwrite(tls *libc.TLS, ptr uintptr, size, n uint64, f uintptr) uint64 {
	return uint64(libc.Xfwrite(tls, ptr, libc.Tsize_t(size), libc.Tsize_t(n), f))
}
